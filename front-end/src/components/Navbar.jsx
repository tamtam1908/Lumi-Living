...
const Navbar = () => {
  ...
  // Đóng menu profile khi click bên ngoài
  useEffect(() => {
    ...
    // Lắng nghe sự kiện click toàn màn hình
  ...
  return (
    <div className='flex items-center justify-between py-5 px-5'>
      ...
      <div className='lg:hidden'>
        <button onClick={toggleMobileMenu} className="text-xl text-[#d9a86d] hover:text-[#c6925a]"><FontAwesomeIcon icon={faBars} className="w-5 h-5 cursor-pointer navbar_font"/></button> {/* Hamburger Menu Icon */}
      </div>

      {/* Mobile Menu */}
      {showMobileMenu && (
        // <div className='absolute top-0 right-0 bottom-0 left-0 bg-[#f4f4f4] z-50 main_bg'>
        ...
        //   </ul>
        // </div>
        <div className='absolute top-0 right-0 bottom-0 left-0 z-50 bg-[#201D1C]/95 h-[65%]'>
          <div className="flex justify-between px-5 py-3">
            <button onClick={() => setShowMobileMenu(false)} className="text-lg text-[#d9a86d] hover:text-[#c6925a]">Back</button> {/* Back button */}
          </div>
          <ul className='flex flex-col items-center gap-2 py-5'> {/* Giảm gap để các phần tử gần nhau hơn */}
            <NavLink 
              to='/about' 
              onClick={() => setShowMobileMenu(false)} 
              className='navbar_font w-full text-center py-3'
            >
              VỀ CHÚNG TÔI
            </NavLink>
            <NavLink 
